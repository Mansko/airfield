...
const LABEL_TO_COLOR: Record<string, any> = {
  lips: '#E0E0E0',
  leftEye: '#E0E0E0',
  leftEyebrow: '#E0E0E0',
  leftIris: '#E0E0E0',
...
export class FacialDetectionService {
  ...
  public async estimateFaces(video: HTMLVideoElement, flipHorizontal?: boolean): Promise<Face[]> {
    let faces: Face[] = [];
    if(this.detector === null) this.detector = await faceLandmarksDetection.createDetector(faceLandmarksDetection.SupportedModels.MediaPipeFaceMesh, DETECTOR_CONFIG);
    try {
      faces = await this.detector.estimateFaces(video, {flipHorizontal: flipHorizontal || false});
    } catch (error) {
  ...
  public drawResults(video: HTMLVideoElement, ctx: CanvasRenderingContext2D | null, faces: Face[], triangulateMesh: boolean, boundingBox: boolean) {
    ...
    faces.forEach((face) => {
      ...
      if (keypoints.length > NUM_KEYPOINTS) {
        ...
        const leftDiameterY =
          this.distance(keypoints[NUM_KEYPOINTS + 4], keypoints[NUM_KEYPOINTS + 2]);
        const leftDiameterX =
          this.distance(keypoints[NUM_KEYPOINTS + 3], keypoints[NUM_KEYPOINTS + 1]);
  ...
  private drawCtx(ctx: CanvasRenderingContext2D | null, video: HTMLVideoElement) {
  ...
  private drawPath(ctx: CanvasRenderingContext2D, points: number[][], closePath: boolean) {
